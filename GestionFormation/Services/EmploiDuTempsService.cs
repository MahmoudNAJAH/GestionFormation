...
namespace GestionFormation.Services
{
    ...
    {
        public static List<JourneeDTO> GetWeekTuple(List<JourneeDTO> allDays, DateTime dateReference)
        {
            List<JourneeDTO> result = new List<JourneeDTO>();

            // -- On veut envoyer Les tuples qui sont la même semaine que ma dateReference
            ...
            //En partant de DateTime.Now, on regarde jusqu'a +7 jours et -7 jours pour être sur d'avoir toute la semaine
            ...
            for (int i_jours = 0; i_jours < 7; i_jours++)
            {
                ...
                {
                    result.AddRange(allDays.FindAll(it => it.Date == nowMinus));
                }
                ...
                {
                    result.AddRange(allDays.FindAll(it => it.Date == nowAdd));
                }
            }
