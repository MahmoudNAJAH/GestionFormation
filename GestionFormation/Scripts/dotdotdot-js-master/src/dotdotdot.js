...
export default class Dotdotdot {
     ...
     */
    constructor(container, options = Dotdotdot.options) {
    ...
    _getOriginalContent() {
        ...
        let _block_tags_ = 'div, section, article, header, footer, p, h1, h2, h3, h4, h5, h6, table, td, td, dt, dd, li';
        /** HTML tags that only have block level children. */
        let _block_parents_ = 'table, thead, tbody, tfoot, tr, dl, ul, ol, video';
        [this.container, ...Dotdotdot.$.find('*', this.container)].forEach((element) => {
            ...
            Dotdotdot.$.contents(element).forEach((text) => {
                ...
                if (text.nodeType == 3) {
                    if (text.textContent.trim() == '') {
                        ...
                        if (text.parentElement.matches(_block_parents_) ||
                            !prev ||
                            (prev.nodeType == 1 &&
